...
class VectorTime:
  ...
  def is_vtime_earlier(self, vtime: VectorTime) -> bool:
    # We don't use Python's __lt__ here since vector times do not have log_state total order.
    # It is possible that (not log_state < b) and (not b < log_state)
...
class Storage:
  ...
  def chk_invariants(self) -> None:
    ...
    # print(f"Commit vector: {self.c}, Tentative vector: {self.f}")
  ...
  def apply(self, commits: list[LogEntry], tentatives: SortedList[LogEntry]) -> None:
    ...
    with self._apply_lock:
      # print("Starting the apply process.")
      # print(f"Initial tentative log: {self.tentative_log}")
      for log_entry in reversed(self.tentative_log):
          log_entry.undo(self.tentative_st)
         
      new_committed_enteries = [log_state for log_state in commits if (log_state in self.committed_log) is False] 
      for log_entry in new_committed_enteries:
          if not self.c.is_ltime_earlier(log_entry.ltime): 
              log_entry.do(self.committed_st)
              log_entry.do(self.tentative_st)
              self.committed_log.append(log_entry)
              self.c.advance(log_entry.ltime)
              self.f.advance(log_entry.ltime)
              
      new_tentative_entries = [log_state for log_state in commits if log_state not in self.committed_log]
      new_tentative_log = SortedList(key=lambda l: l.ltime)
      for log_entry in self.tentative_log:
          if (log_entry in self.committed_log) is False:
              new_tentative_log.add(log_entry)
      self.tentative_log = new_tentative_log
      
      for log_entry in tentatives:
          if not self.f.is_ltime_earlier(log_entry.ltime):
              self.tentative_log.add(log_entry)
              
      for log_entry in self.tentative_log:
          log_entry.do(self.tentative_st)  
          self.f.advance(log_entry.ltime)  
             
    #print("Committed log after applying commits:", self.committed_log)
    #print("Tentative log after applying commits and tentatives:", self.tentative_log)
    self.chk_invariants()
  ...
  def anti_entropy(self, c: VectorTime, f: VectorTime) -> tuple[list[LogEntry], SortedList[LogEntry]]:
    ...
    if not self.c.is_vtime_earlier(c):
         assert c.is_vtime_earlier(self.c)

    # Collect missing committed log entries
    missing_commits = [log_entry for log_entry in self.committed_log 
                       if not c.is_ltime_earlier(log_entry.ltime)]

    # Collect missing tentative log entries
    missing_tentatives = SortedList([log_entry for log_entry in self.tentative_log 
                                    if not f.is_ltime_earlier(log_entry.ltime)],
                                    key=lambda l: l.ltime)

    return missing_commits, missing_tentatives
